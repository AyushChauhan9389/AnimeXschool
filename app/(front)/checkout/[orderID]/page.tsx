...
export default async function OrderPage(props: OrderPageProps) {
      ...
      <div className="h-96 bg-white flex flex-col justify-center items-center p-4">
        <div className="bg-white p-8 rounded-lg shadow-md text-center">
          ...
          <p className="text-gray-600 mb-4">
            The order you are looking for does not exist or you do not have access to view it.
          </p>
  ...
  const OrderAddress = await db
    .select()
    .from(address)
    .where(eq(address.addressId, orderData.addressId));
        ...
        <div className="space-y-6">
          ...
          <div className="border-b pb-4">
            ...
            <p className="text-gray-600">
              Status: {orderData.status} <span className="font-medium text-green-600"></span>
            </p>
          </div>
          ...
          <div className="border-b pb-4">
            ...
            <p className="text-gray-600">
              {OrderAddress[0]?.addressLine1}, {OrderAddress[0]?.addressLine2},{' '}
              {OrderAddress[0]?.city}, {OrderAddress[0]?.state},{' '}
              {OrderAddress[0]?.country}, {OrderAddress[0]?.pinCode}
            </p>
          </div>
        </div>
